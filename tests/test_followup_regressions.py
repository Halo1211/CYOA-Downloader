...
def test_cyoap_successful_asset_fallback_does_not_report_missing_asset(monkeypatch, tmp_path):
    start_url = "https://example.invalid/game/"

    def fake_fetch(url, **_kwargs):
        if url.endswith("platform.json"):
            return _response(url, b'{"image":"cover.webp"}', "application/json")
        if url.endswith("list.json"):
            return _response(url, b"[]", "application/json")
        if url.endswith("/cover.webp") and "/dist/images/" not in url:
            return _response(url, b"image", "image/webp")
        if url == start_url:
            return _response(url, b"<html><body>ok</body></html>", "text/html")
        if url.endswith("favicon.ico"):
            return _response(url, b"icon", "image/x-icon")
        response = _response(url, b"missing", "text/plain")
...
def test_cyoap_recovers_numeric_suffix_after_image_extension(monkeypatch, tmp_path):
    start_url = "https://example.invalid/game/"

    def fake_fetch(url, **_kwargs):
        if url.endswith("platform.json"):
            return _response(url, b'{"image":"cover.webp123"}', "application/json")
        if url.endswith("list.json"):
            return _response(url, b"[]", "application/json")
    ...
    assert json.loads((tmp_path / "game" / "dist" / "platform.json").read_text(encoding="utf-8"))["image"] == "cover.webp"
    assert "Failed       : 0" in (tmp_path / "backup_report.txt").read_text(encoding="utf-8")
