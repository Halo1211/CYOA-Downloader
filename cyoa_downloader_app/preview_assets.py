...
_INT_CYOA_ENHANCER_INFO = {
    ...
    "author": "agreg",
    "version_seen": "0.7",
    "bundled_helper_version": "7.6.0-native-cheat",
    "license": "MIT",
...
(function(){
  'use strict';
  if (window.__CYOA_BUNDLED_INTCYOAENHANCER_V760__) return;
  window.__CYOA_BUNDLED_INTCYOAENHANCER_V760__ = true;
  ...
  const ORIGINALS = { points:new WeakMap(), req:new WeakMap(), rows:new WeakMap(), scores:new WeakMap(), choices:new WeakMap(), disabled:new Map() };
  let choiceFilter = '';
...
`; document.head.appendChild(st); }
  ...
  function snapshotFields(target, names){ const saved={}; names.forEach(name=>{saved[name]={exists:Object.prototype.hasOwnProperty.call(target,name),value:target[name]};}); return saved; }
  function restoreFields(target, saved){ for(const [name,field] of Object.entries(saved)){ if(field.exists)target[name]=field.value; else delete target[name]; } }
  function storeOriginals(){ pointTypes().forEach(p=>{ if(p && !ORIGINALS.points.has(p)) ORIGINALS.points.set(p, snapshotFields(p,['startingSum'])); }); rows().forEach(r=>{if(r && !ORIGINALS.rows.has(r))ORIGINALS.rows.set(r,snapshotFields(r,['allowedChoices','allowedChoicesChange','isEditModeOn']));}); items().forEach(o=>{ if(o && !ORIGINALS.req.has(o)) ORIGINALS.req.set(o,snapshotFields(o,['requireds','isNotSelectable','isSelectable','allowedChoicesChange'])); if(o && !ORIGINALS.choices.has(o))ORIGINALS.choices.set(o,snapshotFields(o,['isActive','selected','isSelected','multipleUseVariable'])); (o.scores||[]).forEach(s=>{if(s && !ORIGINALS.scores.has(s))ORIGINALS.scores.set(s,snapshotFields(s,['requireds']));}); }); }
  function setPoint(id, value){ const p=pointTypes().find(x=>String(x.id)===String(id)); if(!p) throw new Error('Point not found: '+id); if(value==='' || value===null || !Number.isFinite(Number(value))) throw new Error('Point value must be a finite number'); p.startingSum=Number(value); return p.startingSum; }
  ...
  function resetPoints(){ pointTypes().forEach(p=>{ const saved=p&&ORIGINALS.points.get(p); if(saved)restoreFields(p,saved); }); renderCheat(); toast('Points reset to first detected values'); }
  function softenDisabledUI(){ $('button:disabled,input:disabled,select:disabled,textarea:disabled').forEach(el=>{try{if(!ORIGINALS.disabled.has(el))ORIGINALS.disabled.set(el,true);el.disabled=false;el.removeAttribute('disabled')}catch(_){}}); }
  function restoreDisabledUI(){ ORIGINALS.disabled.forEach((wasDisabled,el)=>{try{if(wasDisabled)el.disabled=true}catch(_){}}); ORIGINALS.disabled.clear(); }
  function unlockRequirements(){ storeOriginals(); rows().forEach(r=>{ try{delete r.allowedChoicesChange; r.isEditModeOn=false;}catch(_){}; (r.objects||[]).forEach(o=>{ try{o.isNotSelectable=false;o.isSelectable=true;o.requireds=[];}catch(_){}; (o.scores||[]).forEach(s=>{try{s.requireds=[];}catch(_){}}); }); }); document.documentElement.classList.add('cyoa-ice-reveal'); softenDisabledUI(); toast('Requirements/disabled UI softened for this preview'); }
  function unlimitedChoices(){ storeOriginals(); rows().forEach(r=>{try{r.allowedChoices=0;}catch(_){}}); toast('Allowed choices unlocked for this preview'); renderCheat(); }
  function setChoice(o,active){ if(!o)throw new Error('Choice not found'); o.isActive=!!active; if('selected' in o)o.selected=!!active; if('isSelected' in o)o.isSelected=!!active; if(active && o.multipleUseVariable!=null && Number(o.multipleUseVariable)===0)o.multipleUseVariable=1; }
  function toggleChoice(index){ storeOriginals(); const o=items()[index]; setChoice(o,!Boolean(o&&o.isActive)); renderCheat(); }
  function selectAllSoft(){ storeOriginals(); const all=items(); all.forEach(o=>{try{setChoice(o,true)}catch(_){}}); renderCheat(); toast('Selected '+all.length+' choices in the preview state'); }
  function resetChoices(){ items().forEach(o=>{const saved=o&&ORIGINALS.choices.get(o);if(saved)try{restoreFields(o,saved)}catch(_){}}); renderCheat(); toast('Choices restored to first detected values'); }
  function plainTitle(value){ return String(value==null?'':value).replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim(); }
  function applyChoiceFilter(panel){ const needle=choiceFilter.toLowerCase(); $('[data-choice-row]',panel).forEach(row=>{row.style.display=!needle||row.textContent.toLowerCase().includes(needle)?'':'none'}); }
  function restoreRequirements(){ rows().forEach(r=>{ const saved=r&&ORIGINALS.rows.get(r); if(saved)try{restoreFields(r,saved)}catch(_){} }); items().forEach(o=>{ const saved=o&&ORIGINALS.req.get(o); if(saved)try{restoreFields(o,saved)}catch(_){}; (o.scores||[]).forEach(s=>{const scoreSaved=s&&ORIGINALS.scores.get(s);if(scoreSaved)try{restoreFields(s,scoreSaved)}catch(_){}}); }); restoreDisabledUI(); document.documentElement.classList.remove('cyoa-ice-reveal'); toast('Original requirement snapshot restored'); renderCheat(); }
  ...
  function revealDisabledUI(){ const active=document.documentElement.classList.toggle('cyoa-ice-reveal'); if(active)softenDisabledUI(); else restoreDisabledUI(); toast('Reveal UI '+(active?'on':'off')); }
  ...
  function renderCheat(){ css(); storeOriginals(); let d=document.getElementById(CHEAT_ID); if(!d){ d=document.createElement('div'); d.id=CHEAT_ID; document.documentElement.appendChild(d); } const f=findProjectLike(); const pts=pointTypes(); const rs=rows(); const its=items(); let pointRows = pts.length ? pts.map(p=>`<tr><td><code>${escapeHtml(p.id)}</code><div class="small">${escapeHtml(p.name||p.beforeText||'')}</div></td><td><input type="number" data-pt="${escapeAttr(p.id)}" value="${Number(p.startingSum||0)}"></td><td><button data-add="${escapeAttr(p.id)}" data-d="-100">-100</button> <button data-add="${escapeAttr(p.id)}" data-d="-10">-10</button> <button data-add="${escapeAttr(p.id)}" data-d="-1">-1</button> <button data-add="${escapeAttr(p.id)}" data-d="1">+1</button> <button data-add="${escapeAttr(p.id)}" data-d="10">+10</button> <button data-add="${escapeAttr(p.id)}" data-d="100">+100</button></td></tr>`).join('') : '<tr><td colspan="3">No pointTypes found yet. Try opening the CYOA page fully, then click Refresh Detect.</td></tr>'; let choiceRows=its.slice(0,300).map((o,i)=>`<tr data-choice-row="1"><td><code>${escapeHtml(o.id||i)}</code> ${escapeHtml(plainTitle(o.title||o.name||''))}</td><td>${o.isActive?'Selected':'Off'}</td><td><button data-choice="${i}">${o.isActive?'Deselect':'Select'}</button></td></tr>`).join('')||'<tr><td colspan="3">No choices detected.</td></tr>'; d.innerHTML = `<div class="top"><div><h2>🧩 Bundled ICE Cheat Panel</h2><div class="small">Native localhost panel inspired by IntCyoaEnhancer. No Tampermonkey menu needed.</div></div><button class="danger" data-close="1">Close</button></div><div class="status">Detected source: ${escapeHtml(f.name||'-')} | score: ${f.score||0}\nRows: ${rs.length} | Choices/objects: ${its.length} | Point types: ${pts.length}</div><div class="bar"><button class="primary" data-refresh="1">Refresh Detect</button><button class="accent" data-unlock="1">Unlock Requirements</button><button class="accent" data-unlimited="1">Unlimited Choices</button><button class="accent" data-selectall="1">Soft Select All</button><button data-restore="1">Restore Locks</button><button data-resetpts="1">Reset Points</button><button data-resetchoices="1">Reset Choices</button><button data-project="1">Download State</button><button data-ls="1">Export LS</button><button data-idb="1">Export IDB</button><button data-reveal="1">Reveal UI</button></div><h3>Point editor</h3><table><thead><tr><th>Point type</th><th>Value</th><th>Quick add</th></tr></thead><tbody>${pointRows}</tbody></table><h3>Choices</h3><input type="search" data-filter="1" placeholder="Filter choices" value="${escapeAttr(choiceFilter)}"><div class="small">Showing up to 300 of ${its.length} choices</div><table><thead><tr><th>Choice</th><th>State</th><th>Action</th></tr></thead><tbody>${choiceRows}</tbody></table><div class="note">${escapeHtml(CREDIT)}<br>Source: ${escapeHtml(SOURCE)}<br>These changes affect only this localhost browser preview. Reload or clear storage to undo persistent local state.</div>`; d.onclick=function(ev){ const t=ev.target; if(!t)return; try{ if(t.dataset.close)d.remove(); if(t.dataset.refresh)renderCheat(); if(t.dataset.unlock)unlockRequirements(); if(t.dataset.unlimited)unlimitedChoices(); if(t.dataset.selectall)selectAllSoft(); if(t.dataset.restore)restoreRequirements(); if(t.dataset.resetpts)resetPoints(); if(t.dataset.resetchoices)resetChoices(); if(t.dataset.choice!==undefined)toggleChoice(Number(t.dataset.choice)); if(t.dataset.project)downloadProjectData(); if(t.dataset.ls)exportLocalStorage(); if(t.dataset.idb)exportIndexedDB(); if(t.dataset.reveal)revealDisabledUI(); if(t.dataset.add){addPoint(t.dataset.add,Number(t.dataset.d));renderCheat();} }catch(e){console.error(e);alert(String(e));} }; d.onchange=function(ev){ const t=ev.target; if(t&&t.dataset&&t.dataset.pt){try{setPoint(t.dataset.pt,t.value);toast('Point updated: '+t.dataset.pt+' = '+t.value)}catch(e){alert(String(e))}}}; d.oninput=function(ev){const t=ev.target;if(t&&t.dataset&&t.dataset.filter!==undefined){choiceFilter=t.value;applyChoiceFilter(d)}}; applyChoiceFilter(d); try{ var _top=d.querySelector('.top'); if(_top && !d._dragWired){ _iceDrag(d,_top); d._dragWired=true; } }catch(_){} }
